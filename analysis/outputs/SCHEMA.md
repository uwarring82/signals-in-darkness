...
## Files
- `res1_core.json` — dict with keys A (list of [C, s, I_exact, I_lo, ratio, validity_param]), A2 (list of [C, err_at_q0.06, err_at_q0.17, q_for_6pc, q_for_20pc, k]; see notes/2026-09-03-note-01a-errata.md and claim C21), B (list of [C, s, tcc, S2, I_lo, I_halfsum_exact_rk, I_hmm, pert_param, I_ext]), C (dict "C,s,tcc" -> [thetas_rad, exact, lo]), D (list of [tau, tc, c, s2_closed, s2_num, a1_closed, a1_num]), F (list of [C, s, tcc, I_ext, I_mid, delay_ext_mean, delay_ext_se, h_over_I_ext, delay_mid_mean, delay_mid_se, h_over_I_mid]; units shots). In F, `delay_mid_mean` and `delay_mid_se` are `null` where no mid-fringe run reached the threshold within the cap, so the delay is undefined — this is the case at tau_c/c = 20 and 1. See seeds.md.
