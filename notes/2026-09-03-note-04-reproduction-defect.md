...
Written while certifying the repository seed immediately after the import commit, tag `archive-2026-09-03`. Commit hashes are deliberately not quoted in this note: the import commit's message was corrected on the same day and its hash therefore changed. The tag is the stable reference. Four defects are recorded, in decreasing order of consequence. Items 1–3 concern the policy-comparison chain (`sid_run6s.py`); item 4 is unrelated and concerns `sid_run8.py`. Nothing in notes 01–03a is edited; the consequences for the claim ledger are recorded in `ledgers/status.yaml` and pointed back here.
